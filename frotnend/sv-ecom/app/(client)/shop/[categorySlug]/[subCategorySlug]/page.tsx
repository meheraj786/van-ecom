...
const SubCategoryFilterPage = () => {
  ...
  const subcategory = subResponse?.data?.subcategories?.find(
  ...
  const { data: prodResponse, isLoading: isFetchingProducts } = useProducts({
    page: currentPage,
    limit: pageSize,
    subCategoryId: subcategory?.id || "NON_EXISTENT",
  ...
  if (isFetchingCats || isFetchingSubs || isFetchingProducts) {
    return (
        ...
        {subcategory.image ? (
          ...
          <header>
            ...
            <p className="text-gray-500 max-w-2xl leading-relaxed mt-4">
              Explore our handpicked secondary curated collection inside{" "}
              {category.name} &gt; {subcategory.name}.
            </p>
          </header>
        )}
