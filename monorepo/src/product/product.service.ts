...
export class ProductService {
  ...
  private async executeQuery<T>(
    readQuery: () => Promise<T>,
    _unusedFallbackQuery: () => Promise<T>,
    _unusedEmptyCheck: (result: T) => boolean,
  ): Promise<T> {
    return readQuery();
  }
  ...
  async createProduct(dto: CreateProductDto) {
    ...
    this.validateOptionInput(dto.options);
  ...
  async getProducts(query: GetProductsQueryDto) {
    ...
    const execute = async (client: PrismaService) => {
  ...
  async updateProduct(id: string, dto: UpdateProductDto) {
    ...
    if (dto.categoryIds) {
      ...
      const existingCategoryIds = await this.prisma.productCategory.findMany({
        where: {
          productId: id,
        },
        select: {
  ...
  async createVariant(productId: string, dto: CreateVariantDto) {
    ...
    const existingCombination = await this.prisma.productVariant.findFirst({
      where: {
        productId,
        combinationKey,
      },
  ...
  async getVariantsByProduct(productId: string) {
    const execute = (client: PrismaService) =>
