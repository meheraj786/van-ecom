      ...
      <div className="grid grid-cols-2 gap-6 sm:grid-cols-3 lg:grid-cols-4">
        {items.map((item) => (
          <Link
            key={item.id}
            href={getItemHref(item)}
            className="relative aspect-[3/4] rounded-3xl overflow-hidden group shadow-sm hover:shadow-xl transition-all duration-500 bg-gray-50 border border-gray-100"
          >
            <Image
              src={item.image || DEFAULT_FALLBACK_IMAGE}
              alt={item.name}
